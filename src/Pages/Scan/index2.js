    ...
    if (gUM && cam) {
      try {
        ...
        try {
          ...
          // return;
          window.BarcodeDetector.detect(myImageData)
            .then(barcodes => {
              bCode = barcodes.map(barcode => barcode.rawValue).join(', ');
              document.getElementById("result").innerHTML = bCode;
              if (bCode.toString().length > 5) {
                read(bCode.toString());
                // textCont.textContent = bCode.toString();
              } else {
                setTimeout(captureToCanvas, 50);
              }

              // if(bCode != "" && bCode.length > 1 && bCode != null)  {
              //   document.querySelector("#barcodes").textContent = bCode;
              //   stopStreamedVideo(canvas);
              // } else {
              //   step(capturer);
              // }
            })
